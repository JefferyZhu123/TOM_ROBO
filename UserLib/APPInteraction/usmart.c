...
void usmart_reset_runtime(void)
{
    ...
    __HAL_TIM_SET_AUTORELOAD(&htim7,0XFFFF);     //将重装载值设置到最大
    __HAL_TIM_SET_COUNTER(&htim7,0);             //清空定时器的CNT
...
{
	if(__HAL_TIM_GET_FLAG(&htim7,TIM_FLAG_UPDATE)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=__HAL_TIM_GET_COUNTER(&htim7);
